...
void MultibandCompressor::initialize(double sampleRate, int maxBlockSize) {
    m_sampleRate = sampleRate;
    
    for (int i = 0; i < kNumBands; ++i) {
        m_bandBuffers[i].setSize(2, maxBlockSize);
    }
...
void MultibandCompressor::process(juce::AudioBuffer<float>& buffer) {
    ...
    // Clear band buffers
    for (int i = 0; i < kNumBands; ++i) {
        m_bandBuffers[i].clear();
    }
    
    // Split into bands (simplified - production would use Linkwitz-Riley filters)
    for (int ch = 0; ch < buffer.getNumChannels(); ++ch) {
        const float* input = buffer.getReadPointer(ch);
        
        // Band 0: < crossover[0]
        float* band0 = m_bandBuffers[0].getWritePointer(ch);
        // Band 1: crossover[0] - crossover[1]
        float* band1 = m_bandBuffers[1].getWritePointer(ch);
        // Band 2: crossover[1] - crossover[2]
        float* band2 = m_bandBuffers[2].getWritePointer(ch);
        // Band 3: > crossover[2]
        float* band3 = m_bandBuffers[3].getWritePointer(ch);
        
        // Simplified band splitting (production needs proper crossover filters)
        for (int i = 0; i < numSamples; ++i) {
            band0[i] = input[i] * 0.25f;
            band1[i] = input[i] * 0.25f;
            band2[i] = input[i] * 0.25f;
            band3[i] = input[i] * 0.25f;
        }
    }
    
    // Process each band
    for (int band = 0; band < kNumBands; ++band) {
        const auto& settings = m_bands[band];
        
        if (!settings.enabled || settings.mute)
            continue;
        
        // Simple compression (placeholder)
        float threshold = juce::Decibels::decibelsToGain(settings.threshold);
        
        for (int ch = 0; ch < 2; ++ch) {
            float* data = m_bandBuffers[band].getWritePointer(ch);
            
            for (int i = 0; i < numSamples; ++i) {
                float level = std::abs(data[i]);
                
                if (level > threshold) {
                    float excess = level - threshold;
                    float reduction = excess * (1.0f - 1.0f / settings.ratio);
                    data[i] *= (1.0f - reduction / level);
                }
            }
        }
        
        // Apply makeup gain
        float makeupGain = juce::Decibels::decibelsToGain(settings.makeupGain);
        m_bandBuffers[band].applyGain(makeupGain);
    }
    ...
    for (int band = 0; band < kNumBands; ++band) {
        ...
        if (settings.solo) {
            buffer.makeCopyOf(m_bandBuffers[band], true);
            return;
        }
        
        if (!settings.mute) {
            for (int ch = 0; ch < buffer.getNumChannels(); ++ch) {
                buffer.addFrom(ch, 0, m_bandBuffers[band], ch, 0, numSamples);
            }
        }
    }
}
...
void MultibandCompressor::reset() {
    for (auto& buf : m_bandBuffers) {
        buf.clear();
    }
}

void MultibandCompressor::setCrossover(int index, float frequency) {
    if (index >= 0 && index < 3) {
        m_crossovers[index] = juce::jlimit(20.0f, 20000.0f, frequency);
        updateFilters();
    }
}

float MultibandCompressor::getCrossover(int index) const {
    return (index >= 0 && index < 3) ? m_crossovers[index] : 0.0f;
}

void MultibandCompressor::setBandSettings(int band, const BandSettings& settings) {
    if (band >= 0 && band < kNumBands) {
        m_bands[band] = settings;
    }
}

const MultibandCompressor::BandSettings& MultibandCompressor::getBandSettings(int band) const {
    static BandSettings dummy;
    return (band >= 0 && band < kNumBands) ? m_bands[band] : dummy;
}
